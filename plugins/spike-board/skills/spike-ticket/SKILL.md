...
2. **Self-assessment against the exit questions.** Look the topic up in `${CLAUDE_PLUGIN_ROOT}/references/topic-catalog.md` (read it now if you haven't). If the topic is in the catalog, show the mentee its exit questions and ask which they can already answer confidently. If it isn't in the catalog, draft 4–6 exit questions in the same style — concrete, answerable, Working-level, consumer-not-operator — and run the same self-assessment on those. The questions they *can't* answer define this spike; the ones they can answer are skipped, not re-tested.
